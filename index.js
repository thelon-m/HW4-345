...
if (process.env.NODE_ENV != 'test')
{
	(async () => {
		...
		await createRepo(userId,'testtt');
		await createIssue(userId, "HW4-345", "test");
...
{
	let options = getDefaultOptions("/" + owner + "/repos", "POST", { json: { name: repo } });
...
{
	let options = getDefaultOptions("/repos/" + owner + "/" + repo + "/issues", "POST", { json: { issueName: issueBody } });
